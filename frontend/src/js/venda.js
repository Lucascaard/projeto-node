...
vender.addEventListener("click", (event) => {
  event.preventDefault();
    ...
    const idClienteVenda = formVendas.querySelector('input[name="idClienteVenda"]');
  ...
  .then(data => { 
    ...
    document.getElementById("resp").innerHTML = "Venda realizada com sucesso!";

  })
  // parâmetro que vai dentro do bloco catch vai receber o erro da Promise
  .catch(error => {
    console.error(error);
    document.getElementById("resp").innerHTML = "Erro ao realizar venda. Consulte o erro no Console"; 
  });
});